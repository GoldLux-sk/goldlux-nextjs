  ...
  function hrefParams(from?: string, to?: string) {
    ...
    for (const [key, value] of Object.entries(params)) {
      if (value && value.length > 0)
        s = s.concat(`${s.length > 0 ? "&" : "?"}${key}=${value}`);
    }
    return s;
  }
  ...
  return (
    ...
    >
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="bg-white w-screen pt-10"
      >
          ...
          <div className="mt-10 px-3 flex flex-col items-center">
            ...
            {selectedDay.from ? (
              <div className="flex flex-col">
                <Link
                  ...
                  href={`${pathname}${hrefParams(
                    convertDate(selectedDay.from),
                    selectedDay.to ? convertDate(selectedDay.to) : undefined
                  )}`}
                >
                  Potvrdiť dátum
                </Link>
                <Link
                  onClick={() => setSelectedDay({ from: null, to: null })}
              ...
              <div className="text-center font-semibold leading-snug">
                <div className="mt-10 w-[247px] h-[17px] text-zinc-800 text-[24px]">
                  Vyberte si dátum
                </div>
                <div className="mt-5 w-[247px] h-[53px] text-zinc-500 text-[14px]">
                  Pomocou označenia dátumov od do sa vám zobrazia obejdnávky,
                  podľa označených dátumov
                </div>
              </div>
            )}
          </div>
