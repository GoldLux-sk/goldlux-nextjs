...
async function getOrders(customerId: string, dateFrom: string, dateTo: string) {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_PAYLOAD_CMS_URL}/api/orders${stringifiedQuery}`, {
        method: "GET",
        credentials: "include",
        headers: {
...
export default async function OrderComponent({ customerId, dateFrom, dateTo }: { customerId: string, dateFrom: string, dateTo: string }) {
    ...
    return (
        <div className="mt-5 mb-16">
            ...
            {dateFrom?.length > 1 && (
                <h1 className='text-2xl mt-16 mb-5 font-semibold'>Od {formatDate(dateFrom)} {dateTo?.length > 1 && `do ${formatDate(dateTo)}`}</h1>
            )}
